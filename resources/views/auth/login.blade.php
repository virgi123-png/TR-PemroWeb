...
<br>
    ...
    <div class="sub-cont">
        ...
        <form class="form sign-up" method="POST" action="{{ route('register') }}">
            ...
            @if(session('success'))
                <div class="alert alert-success text-center" style="font-size:12px;margin:10px auto;padding:8px;">{{ session('success') }}</div>
            @endif
            <label>
                <span>Name</span>
                <input type="text" name="name"
                       ...
                       required />
                @error('name')
                    <div class="invalid-feedback d-block text-center" style="font-size:11px;">{{ $message }}</div>
                @enderror
            </label>
